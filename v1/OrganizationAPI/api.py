...
""" Description: Function to get courses-summary of an organization
    Input Parameters:
            course_organization: organization whose courses are required
    Output Type: JSON Dictionary with organization name and list of courses and their summary of the organization
    Author: Jay Goswami
    Date of Creation: 19 June 2017
"""


def get_all_courses(course_organization):
    ...
    name = 'course-v1:' + course_organization + '+%'
    query = "select id, display_name, start, end, enrollment_start, enrollment_end from course_overviews_courseoverview where binary id like %s"
    ...
    dict = {'organization':course_organization}
    ...
    for course in courses:
        block = {}
        block['id'] = course[0]
        block['display_name'] = course[1]
        if course[2] == None:
            block['start'] = ""
        else:
            block['start'] = str(course[2].date())
        if course[3] == None:
            block['end'] = ""
        else:
            block['end'] = str(course[3].date())
        if course[4] == None:
            block['enrollment_start'] = ""
        else:
            block['enrollment_start'] = str(course[4].date())
        if course[5] == None:
            block['enrollment_end'] = ""
        else:
            block['enrollment_end'] = str(course[5].date())
        list.append(block)
...
""" Description: Function to get all courses of all organization, calls get_all_courses() to perform task
    Input Parameters:
            None
    Output Type: List of JSON Dictionary with organization name and list of courses and their summary of the organization
    Author: Jay Goswami
    Date of Creation: 19 June 2017
"""
...
def get_all_organization():
    ...
    query = "select distinct(binary display_org_with_default) from course_overviews_courseoverview"
    ...
    for organization in organizations:
        list.append(get_all_courses(organization[0]))

    return list


""" Description: Function to get courses of an organization with count of students for that course
    Input Parameters:
            course_organization: organization whose courses are required
    Output Type: JSON Dictionary with organization name and list of courses of the organization and their summary and count of students
    Author: Jay Goswami
    Date of Creation: 19 June 2017
"""


def get_all_courses_count_students(course_organization):
    ...
    query = "select t1.id, t1.display_name, t1.start, t1.end, t1.enrollment_start, t1.enrollment_end, t2.students from " \
            "(select id, display_name, start, end, enrollment_start, enrollment_end from course_overviews_courseoverview where binary id like %s) t1 " \
            "left join " \
            "(select course_id, COUNT(user_id) as students from student_courseenrollment where binary course_id like %s group by course_id) t2 " \
            "on binary t1.id = t2.course_id"

    mysql_cursor = db_mysql.cursor()
    mysql_cursor.execute(query, (name,name,))
    courses = mysql_cursor.fetchall()
...
""" Description: Function to get all courses with student count of all organization, calls get_all_courses_count_students() to perform task
    Input Parameters:
            None
    Output Type: List of JSON Dictionary with organization name and list of courses and their summary with student count of the organization
    Author: Jay Goswami
    Date of Creation: 19 June 2017
"""


def get_all_organization_count_students():
    try:
        db_mysql = MySQLdb.connect(user=sql_user, passwd=sql_pswd, db=mysql_db) # Establishing MySQL connection
    except:
        print "MySQL connection not established"
    ...
    for organization in organizations:
        list.append(get_all_courses_count_students(organization[0]))
...
""" Description: Function to get quiz level grades of all students in all courses of an organization
    Input Parameters:
            course_organization: name of the organization for which grades of all students of all its courses are required(eg. IITBombayX)
    Output Type : List of all courses of the organization with a list of grades of all students enrolled in the course
    Author: Jay Goswami
    Date of creation: 21 June 2017
"""


def get_all_courses_grades(course_organization):

    try:
        courses = get_all_courses(course_organization)['courses']
    except:
        return None
...
""" Description: Function to get quiz level grades of all students in all courses of all organizations
    Input Parameters:
            None
    Output Type : List of all organizations and all courses of the organization with a list of grades of all students enrolled in the course
    Author: Jay Goswami
    Date of creation: 21 June 2017
"""


def get_all_organization_courses_grades():
    try:
        db_mysql = MySQLdb.connect(user=sql_user, passwd=sql_pswd, db=mysql_db) # Establishing MySQL connection
    except:
        print "MySQL connection not established"
    ...
    for organization in organizations:
        list.append(get_all_courses_grades(organization[0]))

    return list


""" Description: Function to get count of certificates generated for all course of an organization
    Input Parameters:
            course_organization: organization for which count of certificates of all courses is required (ex. IITBombayX)
    Output Type: List of JSON Dictionary of all the course ids and its count of certificate
    Author: Jay Goswami
    Date of Creation: 22 June 2017
"""


def get_organization_certificate_count(course_organization):
    ...
    for course in courses:
        ...
        course_count = {'course_organization': course_organization, 'course_name': course_name, 'course_run': course_run,
                'course_id': course_id, 'certificate_count': int(count[0])}

        list.append(course_count)

    dict['courses'] = list

    return dict

""" Description: Function to get count of certificates generated for all course of all organization
    Input Parameters:
            None
    Output Type: List of organization and JSON Dictionary of all the course ids and its count of certificate
    Author: Jay Goswami
    Date of Creation: 22 June 2017
"""


def get_all_organization_certificate_count():
    ...
    for organization in organizations:
        list.append(get_organization_certificate_count(organization[0]))
